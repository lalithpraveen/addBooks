...
# to make a save first and load books
...
def load_books():
    try:
        ...
        for book in books_dict:
            book_obj = Book(book['id'], book['name'], book['description'], book['isbn'],
...
def update_book(book):
    edited_book = Book(book['id'], book['name'], book['description'], book['isbn'],
                       book['page_count'], book['issued'], book['author'], book['year'],)
    books = load_books()
    if book != None:
        books = list(filter(lambda bk: int(bk.id)
                     != int(edited_book.id), books))
        books.append(edited_book)
        save_books(books)
# add book
...
def add_book(book):
    books = load_books()
    new_book = Book(book['id'], book['name'], book['description'], book['isbn'],
                    book['page_count'], book['issued'], book['author'], book['year'],)
