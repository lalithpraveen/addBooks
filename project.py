...
from deleteDialog import Ui_delete_dialog
from editDialog import Ui_editBookDialog
from layout import Ui_MainWindow
from addDialog import Ui_addBookDialog
...
class MainWindow(QMainWindow, Ui_MainWindow):
    def __init__(self, parent=None):
        ...
        self.setupUi(self)
        self.new_books_btn.pressed.connect(self.show_add_dialog)
        self.load_issued_table()
        self.load_unissued_table()
        self.load_all_books_table()
        self.edit_issued_btn.clicked.connect(
            lambda: self.edit_book(self.issued_books_table))
        self.delete_issued_btn.pressed.connect(
            lambda: self.delete_book(self.issued_books_table)
        )
        self.refresh_issued_btn.clicked.connect(self.load_issued_table)
        self.edit_unissued_btn.clicked.connect(
            lambda: self.edit_book(self.unissued_books_table))
        self.delete_unissued_btn.pressed.connect(
            lambda: self.delete_book(self.unissued_books_table)
        )
        self.refresh_unissued_btn.clicked.connect(self.load_unissued_table)
    ...
    def save_existing_book(self, ui):
        book = {
            ...
            "page_count": int(ui.pagecount_spinBox.text()),
    ...
    def edit_book(self, table):
        ...
        if (selected_row != -1):
            ...
            dialog.ui.pagecount_spinBox.setValue(int(book.page_count))
            dialog.ui.yes_radioButton.setChecked(book.issued)
            if book.issued == False:
                dialog.ui.yes_radioButton.setChecked(True)
    ...
    def save_new_book(self, ui):
        # print(ui.pagecount_spinBox.text())
    ...
    def load_issued_table(self):
        issued_books = lib.get_issued_books()
        self.issued_books_table.setRowCount(len(issued_books))
        for index, book in enumerate(issued_books):
            book = book.to_dict()
            for book_index, attr in enumerate(book):
                self.issued_books_table.setItem(
                    index, book_index, QTableWidgetItem(str(book[str(attr)])))
                self.issued_books_table.item(
                    index, book_index).setFlags(Qt.ItemIsSelectable | Qt.ItemIsEnabled)

    def load_unissued_table(self):
        unissued_books = lib.get_unissued_books()
        self.unissued_books_table.setRowCount(len(unissued_books))
        for index, book in enumerate(unissued_books):
            book = book.to_dict()
            for book_index, attr in enumerate(book):
                self.unissued_books_table.setItem(
                    index, book_index, QTableWidgetItem(str(book[str(attr)])))
                self.unissued_books_table.item(
                    index, book_index).setFlags(Qt.ItemIsSelectable | Qt.ItemIsEnabled)

    def load_all_books_table(self):
        books = lib.load_books()
        self.all_books__table.setRowCount(len(books))
        for index, book in enumerate(books):
            book = book.to_dict()
            for book_index, attr in enumerate(book):
                self.all_books__table.setItem(
                    index, book_index, QTableWidgetItem(str(book[str(attr)])))
                self.all_books__table.item(
                    index, book_index).setFlags(Qt.ItemIsSelectable | Qt.ItemIsEnabled)
        ...
        if self.search_input.text() != "":
            ...
            if book != None:
                ...
                for book_index, attr in enumerate(book_dict):
                    ...
                    self.search_table.item(
                        0, book_index).setFlags(Qt.ItemIsSelectable | Qt.ItemIsEnabled)
